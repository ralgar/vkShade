...
void vkShade::DiagnosticsOverlay::render()
{
    ...
    const bool showEffects =
        m_diagnosticsState->showEffectsStatus.load(std::memory_order_relaxed);
    if (!showPerformance && !showEffects)
        return;
    ...
    if (ImGui::Begin("vkShade HUD###diagnostics-overlay", nullptr, flags))
    {
        ...
        if (showPerformance)
        {
            // Keep the fixed-width timing layout stable as values change; otherwise
            // AlwaysAutoResize makes the right-anchored overlay visibly jitter.
...
void vkShade::DiagnosticsOverlay::render_performance()
{
    ...
    const auto total = m_totalGpuStatistics.get_snapshot();
    const auto effects = m_effectsGpuStatistics.get_snapshot();
    const auto other = m_otherGpuStatistics.get_snapshot();
...
void vkShade::DiagnosticsOverlay::render_effects_status()
{
    const bool enabled = Locator<ConfigManager>::get().internal()
                             .get<bool>("__INTERNAL__", "EffectsEnabled")
                             .value_or(true);
    ImGui::TextUnformatted("Effects");
    ImGui::SameLine();
    ImGui::TextColored(enabled ? UIStyle::Palette::GREEN : UIStyle::Palette::RED,
                       enabled ? "On" : "Off");
}
