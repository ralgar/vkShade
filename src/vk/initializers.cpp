                                                        ...
                                                        VkImageLayout layout /*= VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL*/)
{
    ...
    depthAttachment.clearValue.depthStencil.depth = 0.0f;   // We use reverse-Z, so clear to 0.0 not 1.0.
...
VkImageCreateInfo vkinit::image_create_info(VkFormat format, VkImageUsageFlags usageFlags, VkExtent3D extent)
{
    ...
    // Optimal tiling, which means the GPU is allowed to shuffle the data as it sees fit.
    // We can also use LINEAR here, but the only use case is CPU-readback.
